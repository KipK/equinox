...
- Improve Regulation dashboard readability with raw-only enum fallbacks, centered wrapping values, configurable grid widths, balanced row heights, and a compact two-row SmartPI limitation chain.
...
- bump load-ha-compnenents to 2.0.0
- updated equinox to new load-ha-compnents api
...
- Update ha-better-history component
  . fix invalid graph color handling
  . fix stacked graph x-axis alignment
  . fix timeline ribbon graph grouping
